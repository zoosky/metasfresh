...
public class MTax extends X_C_Tax
{
	...
	/** Postal Codes		*/
	private I_C_TaxPostal[]	m_postals = null;
	 ...
	 */
	public I_C_TaxPostal[] getPostals (boolean requery)
	{
		if (m_postals != null && !requery)
			return m_postals;

		//FR: [ 2214883 ] Remove SQL code and Replace for Query - red1
		String whereClause = I_C_TaxPostal.COLUMNNAME_C_Tax_ID+"=?";
		List<I_C_TaxPostal> list = new Query(getCtx(), I_C_TaxPostal.Table_Name, whereClause,  get_TrxName())
		.setParameters(new Object[]{getC_Tax_ID()})
		...
		if (list.size() > 0) {
			m_postals = new I_C_TaxPostal[list.size ()];
